...
As an example we'll create an integration with Google, where we'll get the user's tokens, and get their user information.
...
When you have created the project, you'll have to set the redirect URI to `http://localhost:3000/api/auth/integrations/google`. This insures that the OAuth flow will redirect back to your application.
...
<Callout type="warning" emoji="⚠️">
  ...
  `http://localhost:3000/something/api/auth/integrations/google`.
</Callout>
