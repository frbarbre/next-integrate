...
To use the your new integration, you can use the `<Integrate />` component found in the [Getting Started Guide]("docs/getting-started#create-an-integrate-components"). The `shop` prop is required when the provider is `shopify`, which is enforced by the `IntegrateProps` type.
