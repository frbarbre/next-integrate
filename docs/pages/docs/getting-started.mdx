...
<Tabs items={["App Router", "Pages Router"]}>
  ...
  export default function Integrate(
    ...
    const {
      ...
      shop, // The merchant's shop domain, required for the shopify provider
      redirect, // Optional, the URL to redirect to after the integration
      base_path, // Optional, the path to the integration route if it's different from the default: (api/auth/[...integration]/route.ts)
      children, // The children of the component
      className, // Optional, the class name of the component
      ...anchorProps // The rest of the props, passed to the anchor tag
    } = props;
  ...
  export default function Integrate(
    ...
    // This function generates the integration URL, which is used in the Link component
    const integration = integrate({ ...props, redirect: redirect || asPath });
