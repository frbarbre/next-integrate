...
<Tabs items={["App Router", "Pages Router"]}>
  ...
  export default function Integrate({
    ...
    // You can also pass base_path: "/something", if the integration route is different from the default: (api/auth/[...integration]/route.ts)
    const integration =
      provider === "shopify"
  ...
  export default function Integrate({
    ...
    const integration =
      provider === "shopify"
        ? integrate({ name, provider, shop, redirect: redirect || asPath })
        : integrate({ name, provider, redirect: redirect || asPath });
